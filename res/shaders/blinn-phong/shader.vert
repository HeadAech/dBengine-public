...
vec3 getScale(mat4 m) {
    ...
    float scaleZ = length(m[2].xyz);
    return vec3(scaleX, scaleY, scaleZ);
}

vec2 scaleUVByNormal(vec3 normal, vec3 scale, vec2 uv) {
    vec2 uvScale;

    if (abs(normal.z) > 0.5)        // front/back face
        uvScale = scale.xy;
    else if (abs(normal.x) > 0.5)   // left/right face
        uvScale = scale.zy;
...
void main()
{
    vs_out.visible = aInstanceVisible;
    ...
    vs_out.LocalNormal = aNormal;

    vec3 scale = getScale(aInstanceMatrix);
    //vec3 worldNormal = normalize(mat3(aInstanceMatrix) * aNormal);
    //vs_out.TexCoords = scaleUVByNormal(N, scale, aTexCoords);
