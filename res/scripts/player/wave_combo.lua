...
function onReady()
	connect_signal("combo2_signal", uuid, function()
		attack()
	end)
end
...
function handleAnim()
	if isComboActive and animator then
		...
		if animProgress >= animationEndProgress then
			...
			animator:PlayTransition("combo2_to_idle")
...
function attack()
	if animator and not isComboActive then
		animator:PlayTransition("idle_to_combo2")
		isComboActive = true
		particlesEmitted = false  
	end
end
function inArea(gameObject)
	...
	if tagN == "Enemy" or tagN == "Boss" then
    	emit_signal("combo_damage", 30, "combo2", gameObject.uuid)
	end
end